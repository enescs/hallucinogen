...
function backendNote(long) {
  if (state.meta.provider === 'mock') {
    return long ? 'Running the mock provider (HLG_MOCK=1) — no model is involved.'
                : 'Mock provider — canned pages, no model';
  }
  if (state.meta.provider === 'claude') {
    return long ? 'Claude is the model (HLG_LLM=claude) — pages arrive over MCP, and the settings below that tune a local model do nothing.'

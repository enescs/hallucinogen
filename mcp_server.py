...
The second is the point. With `OB_LLM=claude` the browser has no model at all --
...
Claude as something Claude is already listening to. Two ways, both here:
    ...
    push    OB_MCP_CHANNEL=1, and this server declares `claude/channel` and pushes
...
BASE = os.environ.get("OB_URL", f"http://127.0.0.1:{PORT}")
CHANNEL = os.environ.get("OB_MCP_CHANNEL") == "1"
...
pages are written by a language model on demand. When it runs with OB_LLM=claude,
that model is you.
...
def spawn_browser() -> subprocess.Popen:
    ...
    """
    env = {**os.environ, "OB_LLM": "claude", "PORT": str(PORT)}
...
async def announce_loop() -> None:
    ...
    Only runs with OB_MCP_CHANNEL=1. The same endpoint the `next_request` tool
    ...
    if name == "browser_status":
        ...
        if status.get("provider") != "claude":
            note = (
                ...
                "written by that, not by you. Restart it with OB_LLM=claude to take over."
            )
